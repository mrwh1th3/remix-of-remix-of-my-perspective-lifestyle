...
const ArticleCard = ({ id, title, category, date, image, size = "small" }: ArticleCardProps) => {
  const getCategoryClass = (cat: string) => {
    ...
    if (normalized.includes("relaciones")) return "tag-relaciones";
    if (normalized.includes("comunicación") || normalized.includes("comunicacion")) return "tag-comunicacion";
    if (normalized.includes("alianzas")) return "tag-alianzas";
    if (normalized.includes("marketing")) return "tag-marketing";
    return "tag-relaciones";
  };
        ...
        <div className="absolute inset-0 p-8 flex flex-col justify-between">
          ...
          <div className="flex items-end justify-between gap-4">
            <div className="flex-1">
              ...
              <h3 className="text-white text-xl md:text-2xl lg:text-3xl font-display font-bold leading-tight tracking-tight">
                {title}
              </h3>
            </div>
          </div>
